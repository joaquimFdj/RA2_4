...
historicoPorItem listaLogs itemProcurado
    ...
    where
        ...
        iterarHistoricos [] _ = []
        iterarHistoricos (log1:resto) itemProcurado =
            case status log1 of
                Falha _ -> 
                    let codigoItem = case acao log1 of
                                        Add -> drop 22 (detalhes log1)
                                        Remove -> drop 33 (detalhes log1)
                                        Update -> drop 24 (detalhes log1)
                                        Delete -> drop 22 (detalhes log1)
                                        _ -> ""
                    in
                        if codigoItem == itemProcurado
                            then log1 : iterarHistoricos resto itemProcurado
                            else iterarHistoricos resto itemProcurado
                Sucesso ->
                    let codigoItem = case acao log1 of
                                        Add -> drop 17 (detalhes log1)
                                        Remove -> drop 30 (detalhes log1)
                                        Update -> drop 30 (detalhes log1)
                                        Delete -> drop 15 (detalhes log1)
                                        _ -> ""
                    in
                        if codigoItem == itemProcurado
                            then log1 : iterarHistoricos resto itemProcurado
