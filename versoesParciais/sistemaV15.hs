...
import System.IO (openFile, withFile, hPutStr, hClose, hFlush, stdout, IOMode(AppendMode))
...
acaoQuatro inventario = do
    putStr "Insira o ID do Item a ser deletado: "
    hFlush stdout
    codigoItem <- getLine
    ...
    case deletaItem horario inventario codigoItem of
        ...
        Right (novoInv, logSucesso) -> do
            salvarLog logSucesso
            salvarInventario arquivoInv novoInv
            putStrLn "\nItem deletado e salvo com sucesso!\n"
            return novoInv
...
main = do
    ...
    where
        ...
        tratarErroInv _ = do
            putStrLn "Arquivo inicial nao encontrado, gerando arquivo base. \n"
            popularInventario
