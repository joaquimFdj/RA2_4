...
import System.IO (openFile, withFile, hPutStr, hClose, hFlush, stdout, IOMode(AppendMode, WriteMode))
...
-- Função CRUD: Adiciona um novo item no inventário
...
-- Função CRUD: Remove determinada quantidade de estoque de um item especifico
...
-- Função CRUD: Determina uma quantia especifica para um item no estoque
...
-- Recebe o inventario, e o formata para impressao no terminal
...
-- Recebe a lista de logs e a formata para impressao
...
-- A partir da lista de logs, retorna todos os logs relacionados a um determinado item
...
-- Retorna todos os logs de erros da lista de logs
...
-- Determina e retorna o item mais movimentado da lista de logs (sem contar erros)
...
-- Lê o arquivo do inventario, e retorna o inventario
...
-- Salva o estado atual do inventario no arquivo, após qualquer operação que o altere
...
-- Lê o arquivo dos logs, e retorna a lista de Log Entries
...
-- Pega o Log mais recente, e o adiciona no fim do arquivo dos Logs
...
-- Traduz a entrada do usuario para Int, quando o terminal pede algo desse valor
...
-- Função para popular Inventário caso o programa inicie sem arquivo base (tambem cria o arquivo)
...
popularInventario = do 
    ...
    let 
        item1 = Item "001" "Farinha" 5 "Comida"
...
-- Loop principal do sistema, chama as funções das ações a partir da decisao do usuario
...
loopPrincipal inventario = do
    ...
    case opcao of
        ...
        _ -> do
            putStrLn "\n Entrada Invalida! Selecione uma opcao: \n"
            loopPrincipal inventario

-- Coleta entrada do usuario e chama addItem

acaoUm :: Inventario -> IO Inventario
acaoUm inventario = do
    putStr "Insira o ID do Item a ser adicionado: "
    hFlush stdout
    codigoItem <- getLine
    putStr "Insira o nome do Item a ser adicionado: "
    hFlush stdout
    nomeItem <- getLine

    quantidadeItem <- lerQuant "Insira a quantidade inicial do Item a ser adicionado: "
    
    putStr "Digite a categoria do Item a ser adicionado: "
    hFlush stdout
    ...
    case addItem horario item inventario of
        Left (_, logErro) -> do
            salvarLog logErro
            print (status logErro)
            putStrLn " "
            return inventario
        Right (novoInv, logSucesso) -> do
            salvarLog logSucesso
            salvarInventario arquivoInv novoInv
            putStrLn "\nItem adicionado e salvo com sucesso!\n"
            return novoInv

-- Coleta entrada do usuario e chama removeItem

acaoDois :: Inventario -> IO Inventario
acaoDois inventario = do
    putStr "Insira o ID do Item a ter seu Estoque subtraido: "
    hFlush stdout
    codigoItem <- getLine
    ...
    case removeItem horario codigoItem quantidadeItem inventario of
        ...
        Right (novoInv, logSucesso) -> do
            salvarLog logSucesso
            salvarInventario arquivoInv novoInv
            putStrLn "\n Quantidade subtraida e salva com sucesso!\n"
            return novoInv

-- Coleta entrada do usuario e chama updateQty

acaoTres :: Inventario -> IO Inventario
acaoTres inventario = do
    putStr "Insira o ID do Item a ter seu Estoque atualizado: "
    hFlush stdout
    codigoItem <- getLine
    ...
    case updateQty horario inventario codigoItem quantidadeItem of
        ...
        Right (novoInv, logSucesso) -> do
            salvarLog logSucesso
            salvarInventario arquivoInv novoInv
            putStrLn "\nEstoque atribuido e salvo com sucesso!\n"
            return novoInv

-- Chama consultarInventario e imprime todas as entradas

acaoQuatro :: Inventario -> IO()
...
-- Pede a entrada do usuario para o item especifico, e depois chama as 3 funções
-- de analise de logs, e retorna seus resultados respectivos

acaoReport :: [LogEntry] -> IO()
acaoReport listaLogs = do
    putStr "Digite o id do item a ser procurado: "
    hFlush stdout
    codigoItem <- getLine
    putStrLn "\n --== REPORTS DO LOG ==--"
    ...
    case itemMaisMovimentado listaLogs of
        Left erro -> print erro
        Right codigoItem -> print ("Item: " ++ codigoItem)
...
-- MAIN - Trata casos onde o programa roda sem arquivos base, os criando

main :: IO ()
main = do
    putStrLn "---=== INICIO DO SISTEMA ===---\n"

    inventario <- carregarInventario arquivoInv `catch` tratarErroInv
    
    _ <- carregarLogs arquivoLog `catch` tratarErroLog

    loopPrincipal inventario

    where
        tratarErroInv :: IOException -> IO Inventario
        tratarErroInv _ = do
            putStrLn "Arquivo de inventario nao encontrado, gerando arquivo base. \n"
            popularInventario
            
        tratarErroLog :: IOException -> IO [LogEntry]
        tratarErroLog _ = do
            putStrLn "Arquivo de log nao encontrado, gerando arquivo vazio. \n"
            writeFile arquivoLog ""
            return []
